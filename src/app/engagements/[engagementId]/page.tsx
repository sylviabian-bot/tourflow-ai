  ...
  return (
    <div className="space-y-12">
      ...
      {engagement.type === "delegation_visit" ? <>
        <section aria-label="Delegation overview"><MetadataList><MetadataRow label="Engagement stage" value={<StatusBadge tone={engagement.stage}>{formatEngagementStage(engagement.stage)}</StatusBadge>} /><MetadataRow label="Delegation" value={`${engagement.delegationSize ?? 0} synthetic representatives`} /><MetadataRow label={engagement.stage === "follow_up" ? "Visit dates" : "Proposed dates"} value={`${formatDemoDate(engagement.startDate)}–${formatDemoDate(engagement.endDate)}`} /><MetadataRow label="Strategic interests" value={<ul className="flex flex-wrap gap-x-4 gap-y-1">{engagement.strategicInterests.map((interest) => <li key={interest}>{interest}</li>)}</ul>} /></MetadataList></section>
