...
export function EnquiryScopeWorkspace() {
  ...
  const [reviewEdits, setReviewEdits] = useState<ScopeReviewEdits>({});
  ...
  function confirmCurrentScope() {
    if (!draft) return;
    try {
      const confirmed = confirmScope(draft, reviewEdits, excludedObjectiveIds);
      setConfirmedScope(confirmed);
      setWorkflowState("confirmed");
      setError(null);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "This scope cannot yet be confirmed.");
    }
  }
...
function ScopeDraftReview({
  ...
  setReviewEdits: (edits: ScopeReviewEdits) => void;
  onConfirm: () => void;
}) {
  ...
  return (
    <section aria-labelledby="scope-draft-heading" className="border-t border-[var(--divider)] pt-10">
      ...
      <div className="divide-y divide-[var(--divider)]">
        <ReviewField label="Partner" grounding={draft.mentionedOrganisationName.grounding} evidence={draft.mentionedOrganisationName.evidenceExcerpt} explanation={draft.mentionedOrganisationName.inferenceExplanation} edited={hasReviewEdit(reviewEdits, "mentionedOrganisationName")} originalValue={draft.mentionedOrganisationName.value}>
          <input aria-label="Partner organisation" value={effective.mentionedOrganisationName ?? ""} onChange={(event) => setReviewEdits({ ...reviewEdits, mentionedOrganisationName: event.target.value || null })} className="w-full max-w-xl rounded border border-[var(--divider)] bg-[var(--surface)] px-3 py-2 text-sm" />
        ...
        <ReviewField label="Engagement type" grounding={draft.engagementType.grounding} evidence={draft.engagementType.evidenceExcerpt} explanation={draft.engagementType.inferenceExplanation} edited={hasReviewEdit(reviewEdits, "engagementType")} originalValue={draft.engagementType.value}>
          <select aria-label="Engagement type" value={effective.engagementType ?? "unknown"} onChange={(event) => setReviewEdits({ ...reviewEdits, engagementType: event.target.value as EngagementScopeDraft["engagementType"]["value"] })} className="w-full max-w-sm rounded border border-[var(--divider)] bg-[var(--surface)] px-3 py-2 text-sm">
        ...
        <ReviewField label="Delegation size" grounding={draft.delegationSize.grounding} evidence={draft.delegationSize.evidenceExcerpt} explanation={draft.delegationSize.inferenceExplanation} edited={hasReviewEdit(reviewEdits, "delegationSize")} originalValue={draft.delegationSize.value}>
          <input aria-label="Delegation size" type="number" min="1" value={effective.delegationSize ?? ""} onChange={(event) => setReviewEdits({ ...reviewEdits, delegationSize: event.target.value ? Number(event.target.value) : null })} className="w-full max-w-40 rounded border border-[var(--divider)] bg-[var(--surface)] px-3 py-2 text-sm" placeholder="Not provided" />
        </ReviewField>
      </div>
      ...
      <div className="mt-10 flex flex-col gap-3 border-t border-[var(--divider)] pt-6 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs text-[var(--muted)]">{includedObjectiveCount === 0 ? "This incomplete scope remains unconfirmed until at least one grounded objective is available." : "Confirmation is local to this demo session and does not create an Engagement."}</p><button type="button" onClick={onConfirm} disabled={includedObjectiveCount === 0} className="inline-flex min-h-11 items-center justify-center rounded bg-[var(--navy)] px-5 py-2 text-sm font-semibold text-white hover:bg-[var(--brand-strong)] disabled:cursor-not-allowed disabled:opacity-50">Confirm scope</button></div>
    </section>
  );
}

function ReviewField({ label, grounding, evidence, explanation, edited, originalValue, children }: { label: string; grounding: EngagementScopeDraft["engagementType"]["grounding"]; evidence: string | null; explanation: string | null; edited: boolean; originalValue: string | number | null; children: React.ReactNode }) {
  return <section className="grid gap-5 py-6 lg:grid-cols-[12rem_minmax(0,1fr)_minmax(16rem,0.8fr)]"><div><p className="text-sm font-semibold text-[var(--ink)]">{label}</p>{edited ? <p className="mt-2 text-xs font-semibold text-[var(--navy)]">Officer edited</p> : <div className="mt-2"><GroundingIndicator status={grounding} /></div>}</div><div>{children}</div><div>{edited ? <><p className="text-xs text-[var(--muted)]">Original AI extraction</p><p className="mt-1 text-sm font-semibold text-[var(--ink)]">{originalValue ?? "Not provided"}</p><div className="mt-2"><GroundingIndicator status={grounding} /></div></> : null}<div className={edited ? "mt-4" : ""}><Evidence evidence={evidence} explanation={explanation} /></div></div></section>;
}
